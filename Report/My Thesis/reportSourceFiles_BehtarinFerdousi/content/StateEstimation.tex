...
\begin{algorithm}[H]
        ...
        \State $[\underline{x}_k$, $\overline{x}_k]$ $\gets$ \Call{interval}{$\hat{\mathcal{X}}_k$}
...
For every iteration, the order of the zonotope, and hence the computation overhead, increases. It can be limited by reducing the zonotope to a maximum order of $m$ as shown in Line \ref{algl:reduce}.
...
\begin{algorithm}[H]
        ...
        \begin{algorithmic}[1]
        \State $[\underline{e}, \overline{e}]$ $\gets$ \Call{interval}{$\mathcal{S}_x$}$\bigoplus \textbf{S}_w \bigoplus \textbf{S}_v$ \label{alg:main_constraints}
